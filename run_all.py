...
def main():
    ...
    # ─────────────────────────────────────
    # STEP 1-H: 휴장일(공휴일) 확인 — 거래일이 아니면 즉시 정상 종료
    # ─────────────────────────────────────
    # 주말(토·일)은 crontab(월~금)이 거르지만, 날짜가 매년 바뀌는
    # 불규칙한 공휴일·임시공휴일은 crontab으로 막을 수 없다.
    # LS API로 '오늘 장이 열렸는지'를 확인해 휴장일이면 아무 것도 하지 않고 끝낸다.
    if not ls_client.is_trading_day():
        print("[run_all] 오늘은 휴장일(공휴일)입니다 → 자동매매를 건너뜁니다.")
