...
- **동적 목표가(`pending_target`)**: `max(현재가 × 1.02, 240분봉 20MA × 1.005)` — 진입 조건에서 제외, 보조 참고값으로만 유지
...
> 마지막 업데이트: 2026-04-27 (진입 조건 변경 — OR → AND, 터틀 신호 AND 30분 가드 AND 10시 이후 세 조건 모두 충족 시 진입)
