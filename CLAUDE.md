...
- **시간 가드 및 지지 감시**: 최초 돌파 발생 시 **20분 시간 가드**가 작동하며, 가드 시간 도중 `돌파선 - 0.5 * ATR` 마지노선 붕괴 시 타이머 리셋 및 신호 삭제.
  - 마감 시점 판정: 20분 경과 후 `현재가 ≥ 돌파선` 일 때만 최종 매수 진입 (`entry_ready=True`) **AND** 현재 시각 10:00 이상 조건 결합.
  - 리셋 마진 정책: 타이머 리셋 시, 다음 재도전 기준가를 `최고점 + 0.2 * ATR`로 상향하여 횡보장 무한 굴레 방지.
...
### 청산 및 손절
- **하드 손절(고정 2N Stop)**: 진입/피라미딩 시점의 마지막 매수가 - 2N 하락 시 전량 즉시 매도.
  장중에 손절가를 추가로 상향하지 않고 고정 손절가로 유지한다.
  `risk_guardian.py`가 10분 주기 실행마다 손익 및 다른 매도 조건들을 감시한다.
