...
#   [2] timer_agent._check_pullback_retest — 풀백 재돌파 + 10시 필터
#   [3] timer_agent.run_timer_check — 터틀 AND 풀백 재돌파 진입 신호 통합
...
# ══════════════════════════════════════════════════════════
# 테스트 2: timer_agent._check_pullback_retest
#           풀백 재돌파 + 10시 필터
# ══════════════════════════════════════════════════════════

class TestCheckPullbackRetest(unittest.TestCase):
    """_check_pullback_retest — entry_ready AND 10시 이후 조건 확인"""

    WL = {"005930": {"name": "삼성전자"}}

    def _run(self, fixed_now: datetime, entry_ready) -> bool:
        """_now_kst를 고정 시각으로 대체하고 _check_pullback_retest 실행."""
        unheld = {"005930": {"turtle_s1_entry_ready": entry_ready}}
    ...
    def test_entry_ready_True_10시이후_True(self):
        """entry_ready=True + 10:30 → True"""
        self.assertTrue(self._run(FIXED_1030, True))

    def test_entry_ready_False_10시이후_False(self):
        """entry_ready=False → False (재돌파 미충족)"""
        self.assertFalse(self._run(FIXED_1030, False))

    def test_entry_ready_True_10시이전_False(self):
        """entry_ready=True이지만 09:50 → False (10시 이전 진입 불가)"""
        self.assertFalse(self._run(FIXED_0950, True))

    def test_entry_ready_없음_False(self):
        """entry_ready 필드 자체가 없음 → False (예외 없음)"""
        unheld = {"005930": {}}
        with patch("timer_agent._now_kst", return_value=FIXED_1030), \
             patch("timer_agent.get_watchlist", return_value=self.WL):
            import timer_agent
            result = timer_agent._check_pullback_retest(
                "005930", "turtle_s1_entry_ready", unheld)
    ...
    def test_종목없음_False(self):
        ...
        with patch("timer_agent._now_kst", return_value=FIXED_1030), \
            ...
            import timer_agent
            result = timer_agent._check_pullback_retest(
                "005930", "turtle_s1_entry_ready", {})
        self.assertFalse(result)


# ══════════════════════════════════════════════════════════
# 테스트 3: timer_agent.run_timer_check
#           터틀 AND 풀백 재돌파 진입 신호 통합
# ══════════════════════════════════════════════════════════
...
class TestRunTimerCheck(unittest.TestCase):
    ...
    def test_S1_entry_ready_True_신호발생(self):
        """S1 신호 + entry_ready=True + 10:30 → TURTLE_S1 신호"""
        unheld = {"035420": {
    ...
    def test_S1_entry_ready_False_신호없음(self):
        """S1 신호 True이지만 entry_ready=False → 신호 없음 (눌림 대기 중)"""
        unheld = {"035420": {
            "turtle_s1_signal":      True,
            "turtle_s2_signal":      False,
            "turtle_s1_peak_price":  50_000,
        ...
        result = self._run(unheld)
        self.assertEqual(result, [], "entry_ready=False → 신호 없음")

    def test_S1_10시이전_진입불가(self):
        """S1 entry_ready=True이지만 09:50 → 신호 없음 (10시 이전)"""
        unheld = {"035420": {
            "turtle_s1_signal":      True,
            "turtle_s2_signal":      False,
            "turtle_s1_peak_price":  50_000,
    ...
    def test_S2_entry_ready_True_신호발생(self):
        """S2 신호 + entry_ready=True + 10:30 → TURTLE_S2 신호"""
        unheld = {"000660": {
            ...
            "turtle_s1_peak_locked": False,
            "turtle_s1_entry_ready": False,
            "turtle_s2_peak_price":  55_000,
            "turtle_s2_peak_locked": True,
            "turtle_s2_entry_ready": True,
        }}
        result = self._run(unheld)
        self.assertEqual(len(result), 1)
    ...
    def test_중복_S2가_S1보다_우선(self):
        """S1 + S2 동시 entry_ready → TURTLE_S2 하나만"""
        unheld = {"005930": {
            "turtle_s1_signal":      True,
            "turtle_s2_signal":      True,
            "turtle_s1_peak_price":  50_000,
