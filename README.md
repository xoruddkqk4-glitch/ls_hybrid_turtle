...
전설적인 **터틀 트레이딩**의 자금 관리 원칙에, 가짜 돌파를 걸러내는 **20분 시간 가드 및 마지노선 지지 감시 + 10시 이후 진입 필터**를 결합한 하이브리드 전략으로 운영됩니다.
...
| **진입 조건** | 20일 또는 55일 신고가 돌파 **AND** 20분 시간 가드 안착(마지노선 -0.5ATR 지지) **AND** 오전 10시 이후 — 세 조건 모두 충족 시 매수 |
...
| **손절** | 진입/피라미딩 시점의 매수가 대비 2N 하락 시 전량 즉시 매도 (고정 2N Stop) |
...
| 조건 | 내용 |
|------|------|
| ① 신고가 돌파 | 20일 신고가(S1) 또는 55일 신고가(S2) 돌파 (최초 돌파 시 타이머 시작) |
| ② 20분 시간 가드 및 지지 감시 | 20분 동안 마지노선(`돌파선 - 0.5 * ATR`) 아래로 단 1회도 붕괴하지 않고, 20분 경과 시점에 돌파선 이상 유지 |
| ③ 10시 이후 | 현재 시각이 오전 10:00 이상 |

* **마지노선 붕괴 및 시간초과 리셋**: 가드 시간 도중 마지노선이 무너지거나, 20분 만료 시점에 돌파선을 회복하지 못하면 즉시 타이머가 리셋되고 신호가 취소됩니다. 이때 다음 진입을 위한 새로운 목표가는 `감시 중 최고가 + 0.2 * ATR`로 상향 조정되어 횡보장 루프를 방지합니다.

**돌파 시각에 따른 실제 진입 시각 예시:**

| 돌파 시각 | 20분 도달 | 실제 진입 |
|-----------|----------|----------|
| 9시 20분 | 9시 40분 (10시 전) | **10시 00분** (10시 최소 보장) |
| 9시 50분 | 10시 10분 | **10시 10분** |
| 10시 30분 | 10시 50분 | **10시 50분** |
...
터틀 진입이 체결되면 텔레그램으로 **최초 신고가 돌파부터 실제 매수까지 걸린 시간**이 함께 전송됩니다. 이는 "돌파 → 시간 가드 안착 → 매수"로 이어지는 시간 가드 과정에 실제로 얼마나 걸렸는지를 보여줍니다.
...
돌파→매수 소요: 23분 (최초 신고가 돌파 후 가드 시간 만료 및 안착까지)
```

최초 돌파 시각은 `unheld_stock_record.json`의 `turtle_s1/s2_breakout_at` 필드에 기록됩니다. 신호가 리셋되면 초기화되며, 새로운 진입 목표가(+0.2*ATR)를 다시 상향 돌파할 때 타이머가 새로 작동합니다.
...
| 조건 | 내용 | 시간 제한 |
|------|------|----------|
| ① 하드 손절 (고정 2N Stop) | 현재가 ≤ 마지막 매수가 - 2N. 진입/피라미딩 시점에 계산된 고정 손절가 사용 | 없음 (매 회차 체크) |
...
- **손절가 옆에 "도달 시 손익"**: 손절가에 닿아 전량 매도될 때 실현되는 손익 = (손절가 - 평균가) × 수량. 피라미딩 상향 등으로 손절가가 평균가 위로 올라가면 **이익 확정**으로 표시됩니다.
