...
def is_trading_day(reference_code: str = "005930") -> bool:
    """오늘 한국 주식시장이 열렸는지(거래일인지) 확인한다.

    LS증권 API에는 '앞으로의 휴장일 달력'을 알려주는 기능이 따로 없다.
    그래서 대표 종목(기본: 삼성전자)의 일봉을 불러와서
    '오늘 날짜 봉이 만들어졌는지'를 보고 거래일 여부를 판단한다.
        - 장이 열린 날  → 오늘 날짜 봉이 생김 → True
        - 휴장일(주말·공휴일·임시공휴일) → 오늘 봉이 안 생김 → False

    주의:
        - 이 판단은 '장이 열린 뒤(오전 9시 이후)'에만 정확하다.
          장 시작 전에는 아직 오늘 봉이 없어 거래일이라도 False가 나온다.
          (그래서 08:40 장-전 배치에는 사용하지 않는다.)
        - 일봉 조회 자체가 실패하면(네트워크·API 오류) '거래일'로 간주한다.
          일시적 오류 때문에 정상 거래일의 손절 감시까지 멈추는 것을 막기 위함이다.

    Args:
        reference_code: 거래일 판단 기준 종목코드 (기본 "005930" 삼성전자)

    Returns:
        True: 오늘 장이 열림(거래일) 또는 조회 실패
        False: 휴장일(오늘 봉이 없음)
    """
    chart = get_daily_chart(reference_code, count=5)

    # 조회 실패 시: 안전하게 '거래일'로 간주한다 (위 주의 참고)
    if not chart:
        print(f"[ls_client] 거래일 확인용 일봉 조회 실패({reference_code}) → 거래일로 간주하고 진행")
        return True

    # 일봉은 날짜 오름차순 정렬이므로 마지막 항목이 가장 최신 봉이다
    latest_date = str(chart[-1]["date"]).strip()  # 예: "20260529"
    today_str   = datetime.now(_KST).strftime("%Y%m%d")

    is_open = (latest_date == today_str)
    if not is_open:
        print(f"[ls_client] 오늘({today_str}) 봉 없음 — 최신 봉 {latest_date} → 휴장일로 판단")
    return is_open
